...
Package scope: `@aegora`.

**Never commit or push `.codegen/` to GitHub.** It is gitignored; keep the tool on disk only.
...
2. Set `package_scope` (e.g. `@acme`) and rename `@aegora/*` packages.
